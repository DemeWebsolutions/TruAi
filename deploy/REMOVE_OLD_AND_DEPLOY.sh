#!/usr/bin/env bash
#
# Gemini.ai Production Deployment — Remove Old & Deploy New
# Run from a STAGING directory (not from DEPLOY_DIR).
#
# Prerequisites:
#   - PHP 8.0+ with sqlite3, curl, json extensions
#   - TruAi repo uploaded to server (e.g. via SCP, SFTP, Git, or Plesk File Manager)
#
# Usage:
#   # 1. Upload TruAi to a staging dir (e.g. /tmp/gemini-deploy or /home/user/truai)
#   # 2. Run:
#   cd /tmp/gemini-deploy
#   chmod +x deploy/REMOVE_OLD_AND_DEPLOY.sh
#   DEPLOY_DIR=/path/to/live ./deploy/REMOVE_OLD_AND_DEPLOY.sh
#
# Plesk: Tools & Settings → Terminal → cd to staging dir → run script
# Recommended: DEPLOY_DIR=/var/www/vhosts/system/gemini.YourDomain.com/htdocs
# Or for PHP built-in server: DEPLOY_DIR=/home/username/gemini-ai
#
...
# Safety: never deploy over the source (would delete script before copy)
...
if [ "$SRC_REAL" = "$DST_REAL" ] || [[ "$DST_REAL" == "$SRC_REAL"/* ]]; then
  ...
  echo "  Use: DEPLOY_DIR=/path/to/live ./deploy/REMOVE_OLD_AND_DEPLOY.sh"
  exit 1
fi
...
# ========== STEP 3: Remove old files completely ==========
echo "[3/6] Removing old deployment..."
rm -rf "$DEPLOY_DIR"/*
rm -rf "$DEPLOY_DIR"/.[!.]* 2>/dev/null || true
# Restore .env from backup (preserves API keys)
...
nohup php -S 0.0.0.0:$PORT router.php > "$DEPLOY_DIR/logs/server.log" 2>&1 &
...
if lsof -ti :$PORT >/dev/null 2>&1; then
  ...
  echo "=========================================="
  echo "  URL: http://154.53.54.169:$PORT/"
  echo "  Root (/) redirects to Gemini portal"
