...
class Router {
    ...
    private function registerRoutes() {
        ...
        $this->routes['DELETE']['/api/v1/task/{id}'] = [$this, 'handleTaskDelete'];
        ...
        $this->routes['GET']['/api/v1/workspace/file/read'] = [$this, 'handleWorkspaceFileRead'];
    ...
    private function handleLogin() {
        ...
        if (isset($data['encrypted_data']) && isset($data['session_id'])) {
            ...
            if ($this->auth->login(null, null, true, $data['encrypted_data'], $data['session_id'])) {
                echo json_encode([
                    ...
                    'encryption' => 'enabled',
                    'requires_password_change' => $this->auth->getRequiresPasswordChange()
                ]);
            } else {
                require_once __DIR__ . '/roma_trust.php';
        ...
        if ($this->auth->login($username, $data['password'])) {
            ...
            // Persist credentials in OS keychain + config.json so biometric
            // login can retrieve them without a separate enrollment password step.
            try {
                require_once __DIR__ . '/ubsas_auth_service.php';
                (new UBSASAuthService())->storeCredentials('truai', $username, $data['password']);
            } catch (Throwable $e) {
                // Non-fatal: keychain storage failure should not block login
                error_log('[UBSAS] storeCredentials failed: ' . $e->getMessage());
            }
    ...
    /**
     * Password change. When session has requires_password_change (first-time/temp password),
     * ROMA is not required so initial setup works. Otherwise requires ROMA + auth + CSRF.
     * Supports plain or encrypted payload (RSA + AES-256-GCM).
     */
    private function handlePasswordChange() {
        require_once __DIR__ . '/roma_trust.php';
        $encryption = $this->auth->getEncryptionService();
        $allowWithoutRoma = $this->auth->getRequiresPasswordChange();
    ...
    /**
     * DELETE /api/v1/task/{id} — permanently remove a task record.
     */
    private function handleTaskDelete($taskId = null) {
        $taskId = $taskId ?? null;
        if (!$taskId) {
            http_response_code(400);
        ...
        $db = Database::getInstance();
        $existing = $db->query(
            "SELECT id, user_id FROM tasks WHERE id = :id",
            [':id' => $taskId]
        );
        if (empty($existing)) {
            http_response_code(404);
        ...
        if ((string)$existing[0]['user_id'] !== (string)$this->auth->getUserId()) {
            http_response_code(403);
            echo json_encode(['error' => 'Forbidden']);
            return;
        }
        $db->execute("DELETE FROM tasks WHERE id = :id", [':id' => $taskId]);
        echo json_encode(['success' => true]);
    }

    /**
     * GET /api/v1/workspace/file/read?path=relative/path — read a workspace file.
     * Path is resolved relative to WORKSPACE_ROOT and constrained to it.
     */
    private function handleWorkspaceFileRead() {
        $relPath = isset($_GET['path']) ? trim((string)$_GET['path']) : '';
        if ($relPath === '') {
            http_response_code(400);
        ...
        if (!$root) {
            http_response_code(500);
            echo json_encode(['error' => 'Workspace root not found']);
            return;
        }
        // Prevent traversal: resolve and confirm it stays inside root
        $full = realpath($root . DIRECTORY_SEPARATOR . ltrim(str_replace('..', '', $relPath), '/\\'));
        ...
        $allowed = [
            'php','html','css','js','ts','json','md','txt','sh','yaml','yml','xml','sql','py','rb'
        ];
    ...
    private function handleBiometricAuth() {
        ...
        if (!$credentials) {
            echo json_encode([
                'success' => false,
                'error'   => 'No stored credentials found. Log in with your password once, then enroll at TruAi → UBSAS Enrollment to enable biometric sign-in.',
            ]);
            return;
        }
    ...
    private function handleBiometricEnroll() {
        ...
        // Basic input validation (Validator returns ['valid' => bool, ...])
        $userCheck = Validator::username($username);
        if (empty($userCheck['valid'])) {
            http_response_code(400);
            echo json_encode(['success' => false, 'error' => 'Invalid username format']);
            return;
        }
        $allowed = ['biometric', 'keychain', 'autofill', 'password', 'masterkey'];
        ...
        // Map enrollment method to the device_type values allowed by the DB constraint:
        // device_type IN ('biometric', 'manual', 'keychain')
        $deviceTypeMap = [
            'biometric' => 'biometric',
            'keychain'  => 'keychain',
            'autofill'  => 'keychain',  // frontend alias for keychain
            'password'  => 'manual',
            'masterkey' => 'manual',
        ];
        $deviceType = $deviceTypeMap[$method] ?? 'manual';
        ...
        if (!empty($deviceFp)) {
            ...
            if (!empty($existing)) {
                $db->execute(
                    ...
                    [':t' => $deviceType, ':id' => (int)$existing[0]['id']]
                );
            } else {
                $db->execute(
                    ...
                    [':uid' => $userId, ':t' => $deviceType, ':fp' => $deviceFp]
                );
            }
        }
