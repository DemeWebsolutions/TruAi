...
class ChatService {
    ...
    private function getAIResponse($message, $model, $metadata = null) {
        ...
        // Load both API keys from settings so OpenAI and Claude both work (Option 2)
        $openaiKey = null;
        $anthropicKey = null;
        try {
            $auth = new Auth();
            if ($auth->isAuthenticated()) {
                ...
                $openaiKey = isset($settings['ai']['openaiApiKey']) ? trim((string)$settings['ai']['openaiApiKey']) : '';
                $anthropicKey = isset($settings['ai']['anthropicApiKey']) ? trim((string)$settings['ai']['anthropicApiKey']) : '';
            }
        } catch (Exception $e) {
        ...
        if ($model === 'auto') {
            try {
                ...
                $isTruAiKey = (strpos($envOpenai, 'sk-svcacct-') === 0) || (strpos($openaiKey ?? '', 'sk-svcacct-') === 0);
                $hasRealOpenAi = (!empty($openaiKey) || !empty($envOpenai)) && !$isTruAiKey;
                if ($hasRealOpenAi) {
                    $model = 'gpt-4';
                } else {
                ...
                $model = (!empty($anthropicKey) ? 'claude-3-sonnet' : 'gpt-4');
            }
        }
